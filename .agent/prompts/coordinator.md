...
4. 각 task에 Codespaces 필수 계약이 있고 시작 evidence가 제출됐는지 확인한다. 사전 승인 예외 없는 로컬 작업은 검증하지 않는다.
5. 예상 토큰 상한이 안전 잔량을 넘으면 독립 검증 가능한 하위 작업으로 나눈다.
6. 환경 증명과 결과 증거를 충족한 작업만 `verified`로 판정한다.
7. 작업 보드와 `docs/coordination/STATUS.md`를 갱신한다.
