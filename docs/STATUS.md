...
| 협업 운영 구조 | 운영 중 | Worker C 연락 두절로 배정 종료, Worker D에 `0.1B-R1` 교정 재수행 배정 |

다음: Worker D가 `0.1B-R1`에서 버전 변경·결정 절차를 독립 재검증한다. 이 결과가 verified/done이 될 때까지 `0.2`와 후속 플랫폼 작업을 시작하지 않는다. 임의 주제로 자동 생성기를 먼저 만들지 않는다.
